...
const loadImage = () => {
    ...
    if(!file) return; // return if user did not select a file
...
filterOptions.forEach(option => {
    option.addEventListener("click", () => { // we add click event listeners to all filter buttons
...
rotateOptions.forEach(option => {
    option.addEventListener("click", () => { // adding click event listener to all rotate&flip buttons
        if(option.id === "left") {
            ...
            rotate += 90; // if clicked btn is right rotate, inrement rotate value by +90
